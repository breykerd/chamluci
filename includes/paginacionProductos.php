...
$resultado=$mysqli->query("SELECT * FROM productos where id_cate= '$idc'" );
...
$countQuery =$resultado->num_rows;
		...
		$productos = $mysqli->query("SELECT * FROM productos where id_cate= $idc ORDER BY id DESC LIMIT $offset,$per_page");
...
?>
                        <img src="<?php echo $urlImg.$mos['img'.$i];?>"  alt="<?php echo $mos['titulo'];?>">
<?php };};?>
                    </a>
                ...
                <div class="p-a10">
                    <h3 class="dez-title text-uppercase tama p-height-1 h5">
                        <a href="Producto/<?php echo $mos['id'].'-'.$mos['url'];?>"><?php echo $mos['titulo'];?></a>
                    </h3>
