...
/**
 * Table class for laempresa
 */
class laempresa extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'laempresa';
		$this->TableName = 'laempresa';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`laempresa`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('laempresa', 'laempresa', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'IMAGE', 'NO');
		...
		// img
		$this->img = new DbField('laempresa', 'laempresa', 'x_img', 'img', '`img`', '`img`', 200, -1, TRUE, '`img`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->img->Sortable = TRUE; // Allow sort
		$this->fields['img'] = &$this->img;

		// titulo
		$this->titulo = new DbField('laempresa', 'laempresa', 'x_titulo', 'titulo', '`titulo`', '`titulo`', 200, -1, FALSE, '`titulo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->titulo->Sortable = TRUE; // Allow sort
		$this->fields['titulo'] = &$this->titulo;

		// detalle
		$this->detalle = new DbField('laempresa', 'laempresa', 'x_detalle', 'detalle', '`detalle`', '`detalle`', 201, -1, FALSE, '`detalle`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->detalle->Sortable = TRUE; // Allow sort
		$this->fields['detalle'] = &$this->detalle;

		// keywords
		$this->keywords = new DbField('laempresa', 'laempresa', 'x_keywords', 'keywords', '`keywords`', '`keywords`', 201, -1, FALSE, '`keywords`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->keywords->Sortable = TRUE; // Allow sort
		$this->fields['keywords'] = &$this->keywords;

		// description
		$this->description = new DbField('laempresa', 'laempresa', 'x_description', 'description', '`description`', '`description`', 201, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->description->Sortable = TRUE; // Allow sort
		$this->fields['description'] = &$this->description;

		// tituloimg
		$this->tituloimg = new DbField('laempresa', 'laempresa', 'x_tituloimg', 'tituloimg', '`tituloimg`', '`tituloimg`', 201, -1, FALSE, '`tituloimg`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->tituloimg->Nullable = FALSE; // NOT NULL field
		$this->tituloimg->Required = TRUE; // Required field
		$this->tituloimg->Sortable = TRUE; // Allow sort
		$this->fields['tituloimg'] = &$this->tituloimg;
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`laempresa`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->tituloimg->DbValue = $row['tituloimg'];
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "laempresaedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "laempresaadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("laempresaview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("laempresaview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "laempresaadd.php?" . $this->getUrlParm($parm);
		else
			$url = "laempresaadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("laempresaedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("laempresaadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("laempresadelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->tituloimg->setDbValue($rs->fields('tituloimg'));
	}
	...
	public function renderEditRow()
	{
		...
		$this->tituloimg->EditValue = $this->tituloimg->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->tituloimg);
				} else {
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->tituloimg);
					} else {
