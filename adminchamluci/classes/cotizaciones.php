...
/**
 * Table class for cotizaciones
 */
class cotizaciones extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'cotizaciones';
		$this->TableName = 'cotizaciones';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`cotizaciones`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('cotizaciones', 'cotizaciones', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// titulo
		$this->titulo = new DbField('cotizaciones', 'cotizaciones', 'x_titulo', 'titulo', '`titulo`', '`titulo`', 200, -1, FALSE, '`titulo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->titulo->Sortable = TRUE; // Allow sort
		$this->fields['titulo'] = &$this->titulo;

		// img
		$this->img = new DbField('cotizaciones', 'cotizaciones', 'x_img', 'img', '`img`', '`img`', 200, -1, TRUE, '`img`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->img->Sortable = TRUE; // Allow sort
		$this->img->ImageResize = TRUE;
		$this->fields['img'] = &$this->img;

		// cantidad
		$this->cantidad = new DbField('cotizaciones', 'cotizaciones', 'x_cantidad', 'cantidad', '`cantidad`', '`cantidad`', 200, -1, FALSE, '`cantidad`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cantidad->Sortable = TRUE; // Allow sort
		$this->fields['cantidad'] = &$this->cantidad;

		// codigo
		$this->codigo = new DbField('cotizaciones', 'cotizaciones', 'x_codigo', 'codigo', '`codigo`', '`codigo`', 200, -1, FALSE, '`codigo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getMasterFilter()
	{
		...
		// Master filter
		$masterFilter = "";
		if ($this->getCurrentMasterTable() == "clientes") {
			if ($this->codigo->getSessionValue() <> "")
				$masterFilter .= "`codigo`=" . QuotedValue($this->codigo->getSessionValue(), DATATYPE_STRING, "DB");
			else
				return "";
		}
		return $masterFilter;
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "clientes") {
			if ($this->codigo->getSessionValue() <> "")
				$detailFilter .= "`codigo`=" . QuotedValue($this->codigo->getSessionValue(), DATATYPE_STRING, "DB");
			else
				return "";
		}
		return $detailFilter;
	...
	// Master filter
	public function sqlMasterFilter_clientes()
	{
		return "`codigo`='@codigo@'";
	}

	// Detail filter
	public function sqlDetailFilter_clientes()
	{
		return "`codigo`='@codigo@'";
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`cotizaciones`";
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "cotizacionesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "cotizacionesadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("cotizacionesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("cotizacionesview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "cotizacionesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "cotizacionesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("cotizacionesedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("cotizacionesadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("cotizacionesdelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "clientes" && !ContainsString($url, TABLE_SHOW_MASTER . "=")) {
			$url .= (ContainsString($url, "?") ? "&" : "?") . TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_codigo=" . urlencode($this->codigo->CurrentValue);
		}
		return $url;
	}
