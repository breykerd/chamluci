...
class blog extends DbTable
{
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('blog', 'blog', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// img
		$this->img = new DbField('blog', 'blog', 'x_img', 'img', '`img`', '`img`', 200, -1, TRUE, '`img`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->img->Sortable = TRUE; // Allow sort
		$this->img->ImageResize = TRUE;
		$this->fields['img'] = &$this->img;

		// titulo
		$this->titulo = new DbField('blog', 'blog', 'x_titulo', 'titulo', '`titulo`', '`titulo`', 200, -1, FALSE, '`titulo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->titulo->Sortable = TRUE; // Allow sort
		$this->fields['titulo'] = &$this->titulo;

		// detalle
		$this->detalle = new DbField('blog', 'blog', 'x_detalle', 'detalle', '`detalle`', '`detalle`', 201, -1, FALSE, '`detalle`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->detalle->Sortable = TRUE; // Allow sort
		$this->fields['detalle'] = &$this->detalle;

		// detalle2
		$this->detalle2 = new DbField('blog', 'blog', 'x_detalle2', 'detalle2', '`detalle2`', '`detalle2`', 201, -1, FALSE, '`detalle2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->detalle2->Sortable = TRUE; // Allow sort
		$this->fields['detalle2'] = &$this->detalle2;

		// url
		$this->url = new DbField('blog', 'blog', 'x_url', 'url', '`url`', '`url`', 200, -1, FALSE, '`url`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->url->Sortable = TRUE; // Allow sort
		$this->fields['url'] = &$this->url;

		// keywords
		$this->keywords = new DbField('blog', 'blog', 'x_keywords', 'keywords', '`keywords`', '`keywords`', 201, -1, FALSE, '`keywords`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->keywords->Sortable = TRUE; // Allow sort
		$this->fields['keywords'] = &$this->keywords;

		// description
		$this->description = new DbField('blog', 'blog', 'x_description', 'description', '`description`', '`description`', 201, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->detalle2->DbValue = $row['detalle2'];
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("blogview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("blogview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "blogadd.php?" . $this->getUrlParm($parm);
		else
			$url = "blogadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("blogedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("blogadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("blogdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->detalle2->setDbValue($rs->fields('detalle2'));
	...
	public function renderListRow()
	{
		...
		// detalle2
		$this->detalle2->ViewValue = $this->detalle2->CurrentValue;
		$this->detalle2->ViewCustomAttributes = "";
		...
		if ($this->img->UseColorbox) {
			...
			$this->img->LinkAttrs["data-rel"] = "blog_x_img";
		...
		// detalle2
		$this->detalle2->LinkCustomAttributes = "";
		$this->detalle2->HrefValue = "";
		$this->detalle2->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// detalle2
		$this->detalle2->EditAttrs["class"] = "form-control";
		$this->detalle2->EditCustomAttributes = "";
		$this->detalle2->EditValue = $this->detalle2->CurrentValue;
		$this->detalle2->PlaceHolder = RemoveHtml($this->detalle2->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->detalle2);
