...
// Create page object
$cotizaciones_edit = new cotizaciones_edit();

// Run the page
$cotizaciones_edit->run();
...
var fcotizacionesedit = currentForm = new ew.Form("fcotizacionesedit", "edit");
...
fcotizacionesedit.validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($cotizaciones_edit->id->Required) { ?>
			elm = this.getElements("x" + infix + "_id");
			if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
				return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $cotizaciones->id->caption(), $cotizaciones->id->RequiredErrorMessage)) ?>");
		<?php } ?>
		<?php if ($cotizaciones_edit->titulo->Required) { ?>
		...
		<?php } ?>
		<?php if ($cotizaciones_edit->img->Required) { ?>
		...
		<?php } ?>
		<?php if ($cotizaciones_edit->cantidad->Required) { ?>
		...
		<?php } ?>
		<?php if ($cotizaciones_edit->codigo->Required) { ?>
...
fcotizacionesedit.Form_CustomValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
...
fcotizacionesedit.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;
...
?>
<form name="fcotizacionesedit" id="fcotizacionesedit" class="<?php echo $cotizaciones_edit->FormClassName ?>" action="<?php echo CurrentPageName() ?>" method="post">
<?php if ($cotizaciones_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $cotizaciones_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cotizaciones">
<input type="hidden" name="action" id="action" value="update">
<input type="hidden" name="modal" value="<?php echo (int)$cotizaciones_edit->IsModal ?>">
...
<?php if ($cotizaciones->id->Visible) { // id ?>
	<div id="r_id" class="form-group row">
		<label id="elh_cotizaciones_id" class="<?php echo $cotizaciones_edit->LeftColumnClass ?>"><?php echo $cotizaciones->id->caption() ?><?php echo ($cotizaciones->id->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $cotizaciones_edit->RightColumnClass ?>"><div<?php echo $cotizaciones->id->cellAttributes() ?>>
<span id="el_cotizaciones_id">
<span<?php echo $cotizaciones->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?php echo RemoveHtml($cotizaciones->id->EditValue) ?>"></span>
</span>
<input type="hidden" data-table="cotizaciones" data-field="x_id" name="x_id" id="x_id" value="<?php echo HtmlEncode($cotizaciones->id->CurrentValue) ?>">
<?php echo $cotizaciones->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cotizaciones->titulo->Visible) { // titulo ?>
	<div id="r_titulo" class="form-group row">
		<label id="elh_cotizaciones_titulo" for="x_titulo" class="<?php echo $cotizaciones_edit->LeftColumnClass ?>"><?php echo $cotizaciones->titulo->caption() ?><?php echo ($cotizaciones->titulo->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $cotizaciones_edit->RightColumnClass ?>"><div<?php echo $cotizaciones->titulo->cellAttributes() ?>>
...
<?php if ($cotizaciones->img->Visible) { // img ?>
	<div id="r_img" class="form-group row">
		<label id="elh_cotizaciones_img" class="<?php echo $cotizaciones_edit->LeftColumnClass ?>"><?php echo $cotizaciones->img->caption() ?><?php echo ($cotizaciones->img->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $cotizaciones_edit->RightColumnClass ?>"><div<?php echo $cotizaciones->img->cellAttributes() ?>>
...
<?php if ($cotizaciones->cantidad->Visible) { // cantidad ?>
	<div id="r_cantidad" class="form-group row">
		<label id="elh_cotizaciones_cantidad" for="x_cantidad" class="<?php echo $cotizaciones_edit->LeftColumnClass ?>"><?php echo $cotizaciones->cantidad->caption() ?><?php echo ($cotizaciones->cantidad->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $cotizaciones_edit->RightColumnClass ?>"><div<?php echo $cotizaciones->cantidad->cellAttributes() ?>>
...
<?php if ($cotizaciones->codigo->Visible) { // codigo ?>
	<div id="r_codigo" class="form-group row">
		<label id="elh_cotizaciones_codigo" for="x_codigo" class="<?php echo $cotizaciones_edit->LeftColumnClass ?>"><?php echo $cotizaciones->codigo->caption() ?><?php echo ($cotizaciones->codigo->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $cotizaciones_edit->RightColumnClass ?>"><div<?php echo $cotizaciones->codigo->cellAttributes() ?>>
...
<div class="form-group row"><!-- buttons .form-group -->
	<div class="<?php echo $cotizaciones_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit"><?php echo $Language->phrase("SaveBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-href="<?php echo $cotizaciones_edit->getReturnUrl() ?>"><?php echo $Language->phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
